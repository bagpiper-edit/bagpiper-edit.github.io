...
def resolve_audio(model, sample_id, meta_rec, sub_id,
    ...
    if explicit:
        ...
        if not expdir_tpl:
            skipped[0] += 1
            return None
        expdir = expdir_tpl.replace("{task}", sub_id)
...
def build_subsection_html(subsection, sec_id, all_models,
    ...
    lines.append(_th(f'{badge}{H(source_model["name"])}'))
    if has_src_cap:
    ...
    for col_name, mids in col_name_to_mids.items():
        ...
        lines.append(_th(f'{badge}{H(col_name)}'))
    ...
    for row_idx, sample in enumerate(samples_cfg, 1):
        ...
        lines.append("<tr>")
        lines.append(_td(H(sid), "utt-id-col sticky-col"))
        if has_text:
            lines.append(_td(H(text), "text-col sticky-col"))
        if has_instruction:
            lines.append(_td(H(instruction), "text-col sticky-col"))
